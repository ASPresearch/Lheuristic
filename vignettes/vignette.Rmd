...
# Introduction
This vignette offers examples on how to use the `LHeuristic` package. The package is designed to help identify genes that exhibit an L-shaped pattern in scatterplots comparing gene expression to methylation.

In essence, the package allows users to:
...
- Data preprocessing may need to be automated to ensure its results can be used for filtering.

- Set parameter values

    - For data use (all genes or only those that pass certain filters ($\mbox{corr}< 0$))
    - For the L-shape selection method, the user may need to choose the method and adjust its parameters, as each method has its own set of parameters.
...
The package includes utility functions like `checkPairing`, which verifies that both matrices have matching row and column numbers and names.
...
We have two aligned datasets, so using an appropriate data structure like `MultiAssayExperiment` is recommended, as outlined in this tutorial.

It is essential to label the experiments as "methylation" and "expression" in this specific order for the `Lheuristic` package functions to work correctly.
...
```{r}
correlation <- correlationSelection(mae1[[1]], mae1[[2]],
    pValCutoff = 0.25, rCutoff = -0.5,
    type = "Spearman", adj = TRUE
)
correlationL <- correlation[correlation$SigNegCorr, ]
correlationNoL <- correlation[!correlation$SigNegCorr, ]

message(
    "The number of genes selected with the correlation method is: ", sum(correlationL$SigNegCorr),
    "\n"
)
```
...
opt <- par(mfrow = c(2, 2))
plotGenesMat(
    mets = mae1[[2]][genes2plot, 1:4], expres = mae1[[1]][genes2plot, 1:4],
...
The heuristic method selects L-shaped scatterplots by overlaying a grid on the graph and defining cells that must contain a minimum (or maximum) percentage of points for the scatterplot to be considered L-shaped.

It computes a score where points in the selected regions (L region) receive positive scores, while points outside these areas receive negative scores. Properly setting scores and weights should yield positive scores for L-shaped scatterplots and negative scores for others.
...
2. Classify the scatterplot as **"L" or "non-L"** based on a few conditions    
    2.1. A *minimum* number of points must be in the upper-left (cell (1,1)) and lower-right (cell (3,3)) corners.
    2.2. A *maximum* number of points must be in the upper-right (cell (1,3)), as points here indicate hypermethylation and hyperexpression, which are contrary to our goal.
    2.3. A minimum of points in cell (3,1) is usually *not required* unless we strictly want an L-shape; we can also accept diagonals that reflect a negative correlation.

3. Score points in each sub-grid as follows:
    3.1. Points in permitted regions (left margin: cells (1,1), (2,2), (3,1), (3,2), (3,3)) score positively if classified as L, or zero if classified as non-L.
    3.2. Points in non-desired regions (outer band: cells (1,2), (1,3), (2,3)) always score negatively.
...
- \(X\) is the matrix of *counts*, representing the number of counts in each cell of the grid.
- \(W_L\) is the matrix of scores per cell and point if the scatterplot is classified as \(L\).
- \(W_{L^c}\) is the matrix of scores per cell and point if the scatterplot is classified as non-\(L\) (\(L^c\)). The symbol \(\circ\) denotes the Hadamard product of the two matrices \(W_{L/L^c}\) (i.e., element-wise multiplication), and \(\mathbf{1}_{L/L^c}()\) is the indicator function for \(L\) or \(L^c\).
...
where:
- \(X\) is the matrix of *counts*, indicating the number of counts in each grid cell.
- \(C\) is the matrix of conditions to verify if the scatterplot should be classified as \(L\).
- \(mMP\) is the matrix of minimum and maximum percentages of points required in each cell for classification as \(L\).
- \(\circ\) represents point-wise logical operations, allowing the product of the three matrices to act as a logical operation.
- \(\bigwedge_{i,j}\) denotes a logical "AND" operation across all cells; if all cells are TRUE, the result is assigned to \(L\); if any cell fails, it is assigned to \(L^c\).
...
opt <- par(mfrow = c(2, 2))
plotGenesMat(
    mets = mae1[[2]][genes2plot2, ], expres = mae1[[1]][genes2plot2, ],
...
inCorrelationLOnly <- setdiff(rownames(correlationL), inCommonL)
...
A heuristic algorithm to select genes potentially regulated by methylation [version 1; not peer reviewed]. 
