...
# Information about features goes to "RowData"
# It seems this property is available in SummarizedExperiments, 
# but not in MultiAssayExperiments

# The main reason seems to be because `MultiAssayExperiment` requires
# the data slots to have the same column numbers & names
# but allows for distinct rows numbers/names
# Check QFeaturesClass

# rowDat <- data.frame(featureID = rownames(methylData), 
#                      isOncogene =c(rep(1,3), rep(0,7)),
#                      known2Methylate =sample(x= c(0,1), size= 10, rep=TRUE, 
#                                              prob=c(.8,.2)))
# rownames(rowDat)<- rowDat$featureID
