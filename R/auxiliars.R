...
matAllCorrs <- function(X, Y, sortByCorrs = FALSE) {
    ...
    colnames(corL) <- c("r (Sp)", "r (Pear)", "distCor", "p (Sp)", "p (Pear)")
    for (i in seq_len(nrow(X))) {
        corrs <- allCorrs(X[i, ], Y[i, ])
        corL[i, ] <- corrs
    }
    corL <- cbind(corL, adj.Spear.Pval = p.adjust(corL[, "p (Sp)"], "fdr"))
    corL <- cbind(corL, adj.Pear.Pval = p.adjust(corL[, "p (Pear)"], "fdr"))
