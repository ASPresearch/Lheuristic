...
#' @param expmat matrix containing the expression data of the named gene.
...
#'
plotGeneByName <- function(geneName, mets, expmat, filename = NULL, text4Title = NULL,
    ...
    if (!(is.null(genePos))) {
        ...
        plotGeneSel(
            xMet = xVec, yExp = yVec, titleText = text4Title, x1 = 1 / 3, x2 = 2 / 3,
            plotGrid = plotGrid
        )
    }
    if (!is.null(filename)) {
