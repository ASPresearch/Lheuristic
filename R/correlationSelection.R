...
#' )
#'
correlationSelection <- function(X, Y, type = "Spearman", adj = TRUE,
    pValCutoff = 0.05,
    rCutoff = 0, sortByCorrs = FALSE) {
    corsMat <- matAllCorrs(X, Y, sortByCorrs = sortByCorrs)
    if (type == "Spearman") {
        selected <- corsMat[, c("r (Sp)", "p (Sp)", "adj.Spear.Pval",
            "distCor")]
        if (adj) {
            lShaped <- (selected[, "r (Sp)"] < rCutoff) &
                (selected[, "adj.Spear.Pval"] <
                pValCutoff)
        } else {
            lShaped <- (selected[, "r (Sp)"] < rCutoff) &
                (selected[, "p (Sp)"] <
        ...
        selected <- corsMat[, c("r (Pear)", "p (Pear)", "adj.Pear.Pval",
            "distCor")]
        if (adj) {
            lShaped <- (selected[, "r (Pear)"] < rCutoff) &
                (selected[, "adj.Pear.Pval"] <
                pValCutoff)
        } else {
            lShaped <- (selected[, "r (Pear)"] < rCutoff) &
                (selected[, "p (Pe)"] <
